...
\usepackage{smartdiagram}\usesmartdiagramlibrary{additions}
...
%----------------------------------------------------------------------------------------------------------------------%
    ...
    \begin{frame}
        \begin{figure}
        \tikzstyle{block} = [rectangle, draw, fill=orange!60!yellow, text width=3cm,minimum height=1em,
        inner sep=0pt,
        text centered, minimum height=1cm,font=\bfseries]

        \begin{tikzpicture}[node distance = 4cm, auto,>=stealth']

        \node [block] (b) {Create Scala mvn Proj};
        \node [block, right of=b] (c) {Scala static analysis tools};
        \node [block, right of=c] (d) {Add mvn site reporting};
        \node [block, below of=d] (e) {Eliminate boilerplate};
        \node [block, left of=e] (f) {Mvn release strategy};
        \node [block, left of=f] (g) {GitHub actions};
